// logger.ts — KyberVision23Queuer
// Must be the second import in app.ts, immediately after "dotenv/config".
...
// ---------------------------------------------------------------------------
// Formats
// [NAME_APP] prefix is included in all messages, preserving the logging
// convention previously handled by the server.js logger override.
// ---------------------------------------------------------------------------
const consoleFormat = winston.format.combine(
  ...
  winston.format.printf(
    ({ timestamp, level, message }) =>
      `${timestamp} ${level}: [${NAME_APP}] ${message}`
  )
);
...
const fileTransport = new winston.transports.File({
  ...
  tailable: true, // current log stays NAME_APP.log; rotated as NAME_APP1.log, NAME_APP2.log, …
});
